...
# ==========================================
# 4. உள்நுழைவு திரை (Username & Password மட்டும்)
# ==========================================
    ...
    if submitted:
      if username.strip() and password.strip():
        user_query = (
            ...
            .select(
                "id, name, username, role, branch_id, is_active,"
        ...
        if user_query.data:
          ...
          if role in ["Admin", "Auditor"]:
            ...
            b_name = (
                branch_rel.get("branch_name")
                if branch_rel
                else "ஒதுக்கப்படாத கிளை"
            )

          if role not in ["Admin", "Auditor"] and not b_id:
            st.error(
                "உங்களுக்கு இன்னும் கிளை ஒதுக்கப்படவில்லை! நிர்வாகியைத்"
                " தொடர்பு கொள்ளவும்."
            )
          else:
          ...
          st.error(
              "தவறான பயனர் பெயர் அல்லது கடவுச்சொல்! (அல்லது கணக்கு"
              " முடக்கப்பட்டுள்ளது)"
          )
      else:
...
# ==========================================
  ...
  with top_col2:
    st.write(
        f"👤 **பயனர்:** {st.session_state.username}"
        ...
        if users_res.data:
          ...
          with st.form("edit_user_form"):
            ...
            edit_role = st.selectbox(
                "பணி நிலை (Role)", roles_list, index=role_index, key="edit_role"
            )
  ...
  # ----------------------------------------------------
    ...
    response = (
        ...
        .select("*, transactions(*), audit_records(*)")
    ...
    if not pending_visits:
      ...
      for item in pending_visits:
        with st.expander(
            f"வருகை எண்: {item['visit_no']} | நிகர ரொக்கம்:"
            f" ₹{item['net_cash_amount']}"
        ):
          ...
          with col_a1:
            if st.button(
                f"அங்கீகரி (Approve) - {item['visit_no']}", key=f"app_{item['id']}"
            ):
          ...
          with col_a2:
            if st.button(
                "விளக்கம் கேள் (Need Clarification)", key=f"rej_{item['id']}"
            ):
              supabase.table("customer_visits").update(
  ...
  # ----------------------------------------------------
    ...
    # உள்நுழைந்த கிளையின் பணியாளர்கள் மட்டுமே பட்டியலிடப்படுவார்கள்
    staff_res = (
        supabase.table("users")
        .select("name")
    ...
    # படி 2: நடவடிக்கைகள் சேர்த்தல் (Cart)
      ...
      st.success(
          f"வாடிக்கையாளர்: **{visit['customer_name']}** (வருகை எண்:"
          f" {visit['visit_no']})"
      )
      ...
      with st.form("add_txn_form"):
        ...
        with t_col2:
          staff = st.selectbox(
              "கையாண்ட பணியாளர் (Staff Attribution)", current_staff_list
          )
        with t_col3:
          amount_paid = st.number_input(
              "செலுத்தியது (Paid Amount ₹)", min_value=0.0, step=100.0
          )
        with t_col4:
          amount_received = st.number_input(
              "பெற்றது (Received Amount ₹)", min_value=0.0, step=100.0
          )
    ...
    # படி 4: Supabase-ல் ஆவணங்கள் பதிவேற்றம் & தணிக்கைக்கு சமர்ப்பித்தல்
      ...
      uploaded_files = st.file_uploader(
          "ஆவணங்களைத் தேர்ந்தெடுக்கவும் (Pledge Form / Photo / KYC)",
      ...
      if st.button("பரிவர்த்தனையை நிறைவு செய்து தணிக்கையருக்கு அனுப்புக"):
        if uploaded_files:
          with st.spinner("ஆவணங்கள் பதிவேற்றப்பட்டு வருகின்றன..."):
            try:
              doc_links = upload_files_to_supabase(
                  uploaded_files, visit["visit_no"]
              )
              visit_data = {
              ...
              visit_res = (
                  supabase.table("customer_visits").insert(visit_data).execute()
              )
              ...
              st.success(
                  "✅ ஆவணங்கள் பதிவேற்றப்பட்டு தணிக்கையருக்கு (Auditor)"
